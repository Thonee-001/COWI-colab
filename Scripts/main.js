...
function setupNavbarScroll() {
  window.addEventListener('scroll', function() {
    if (window.scrollY > 50) {
      // User has scrolled down — make navbar background transparent for glass effect
      navbar.style.backgroundColor = '#ffffff7e';
      scnavbar.style.backgroundColor = '#ffffff7e'
      navbar.style.boxShadow = '0 4px 30px rgba(0,0,0,0.15)';
      scnavbar.style.boxShadow = '0 4px 30px rgba(0,0,0,0.15);'
    } else {
      // Back at top — restore the glass effect
      //navbar.style.backgroundColor = '';
      navbar.style.boxShadow = '';
      scnavbar.style.boxShadow = '';
    }
  });
}
